...
class CompletionChatBenchmark:
    ...
    def __init__(self, base_url: str = "http://localhost:5002", model: str = "gpt2-small"):
        self.base_url = base_url
        self.model = model
        self.results: Dict[str, List[float]] = {}
        self.token_counts: Dict[str, List[int]] = {}
        
    async def _make_request(self, payload: dict, track_tokens: bool = True) -> Tuple[float, bool, Optional[dict]]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=payload, headers=headers) as response:
                                ...
                                if line_str.startswith('data: ') and line_str != 'data: [DONE]':
                                    ...
                                    if track_tokens and "outputs" in data:
                                        ...
                                        for output in data["outputs"]:
                                            if output.get("type") == "STEERED" and "raw" in output:
                                                result_data["steered"] = output["raw"]
                                            elif output.get("type") == "DEFAULT" and "raw" in output:
                    ...
                    if track_tokens:
                        total_tokens = len(result_data.get("steered", "").split()) + len(result_data.get("default", "").split())
        ...
        for response_type in ["STEERED", "DEFAULT"]:
            ...
            for i in range(iterations):
                ...
                if success:
                    ...
                    console.print(f"    Run {i+1}: {elapsed*1000:.2f}ms ({data['tokens'] if data else '?'} tokens)")
                else:
        ...
        for test_case in test_cases:
            ...
            for i in range(iterations):
                ...
                if success:
                    ...
                    console.print(f"    Run {i+1}: {elapsed*1000:.2f}ms ({data['tokens'] if data else '?'} tokens, {tokens_per_sec:.1f} tok/s)")
                else:
                    console.print(f"    Run {i+1}: [red]Failed[/red]")
    ...
    async def benchmark_stress_test(self):
        ...
        prompt = [
            {"role": "user", "content": "Write a detailed story about artificial intelligence."}
        ]
        ...
        console.print(f"  Testing with {len(features)} steering features, n_completion_tokens=200")
        
        elapsed, success, data = await self._make_request(payload)
        if success:
            tokens_per_sec = data["tokens"] / elapsed if data and elapsed > 0 else 0
            console.print(f"  Result: {elapsed*1000:.2f}ms ({data['tokens'] if data else '?'} tokens, {tokens_per_sec:.1f} tok/s)")
    ...
    def print_summary(self):
        ...
        for test_name, times in self.results.items():
            if times:
                ...
                tokens_per_sec = avg_tokens / statistics.mean(times) if times and avg_tokens > 0 else 0
        ...
        if single_steered and single_default and both_short:
            ...
            console.print(f"  Current overhead: {actual_overhead:.2f}ms ({actual_overhead/avg_both*100:.1f}% of total)")
            console.print(f"  [yellow]Optimization potential: ~{expected_sequential - avg_both:.2f}ms reduction[/yellow]")
        ...
        for test_name, times in self.results.items():
            ...
            if times:
                ...
                output["summary"][test_name] = {
                    ...
                    "avg_tokens": statistics.mean(tokens) if tokens else 0,
                    "tokens_per_sec": statistics.mean(tokens) / statistics.mean(times) if tokens and times else 0
                }
...
async def main():
    parser = argparse.ArgumentParser(description="Benchmark completion-chat endpoint performance")
    parser.add_argument("--endpoint", default="http://localhost:5002", help="Base URL of the inference server")
    parser.add_argument("--model", default="gpt2-small", help="Model ID to use for testing")
    parser.add_argument("--phase", choices=["before", "after"], default="before", help="Benchmark phase")
    ...
    try:
        ...
        # Export results
        filename = benchmark.export_results(phase=args.phase)
